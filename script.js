...
window.onload = function () {
  ...
  document.querySelectorAll("nav ul > li").forEach(element => {
    element.addEventListener("click", (e)=>{
      if (element.attributes.linked) {
        if (element.attributes.newtab && !(element.attributes.newtab.value=="false")) {
          window.open(element.attributes.linked.value, '_blank')
        } else {
          window.location = `${element.attributes.linked.value}`
        }
      }
    })
  })
